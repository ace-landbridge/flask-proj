...
- 🎯 Entry points to execute your program using `python -m <flask_proj>` or `$ flask_proj` with basic CLI argument parsing.
...
---
# flask_proj Flask Application

Awesome flask_proj created by haingtrong
...
```bash
git clone https://github.com/haingtrong/flask-proj flask_proj
cd flask_proj
make install
```
...
```bash
flask_proj create-db   # run once
flask_proj populate-db  # run once (optional)
flask_proj add-user -u admin -p 1234  # ads a user
flask_proj run
```
