...
original_author="haingtrong"
original_name="flask_proj"
original_urlname="flask-proj"
original_description="Awesome flask_proj created by haingtrong" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_proj "${name}"
...
then
    ...
    echo "then run 'flask_proj' or 'python -m flask_proj'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_proj' or 'python -m flask_proj' or 'poetry run flask_proj'"
fi
